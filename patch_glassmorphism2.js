...
const oldSummary = `const SummaryCard = ({ title, value, icon: Icon, shadowClass, textClass }: any) => {
  return (
    <motion.div 
...
const newSummary = `const SummaryCard = ({ title, value, icon: Icon, shadowClass, colorClass }: any) => {
  return (
    <motion.div 
      whileHover={{ y: -4, scale: 1.02 }}
      transition={{ type: "spring", stiffness: 300 }}
